% test_computeEuclideanRDM.m
% This script tests the functionality of RSA.computeRDM.computeEuclideanRDM()
...
try
    dissimilarities = RSA.computeRDM.computeEuclideanRDM(X, Y);
    disp('Test 1 passed.');
catch
    disp('Test 1 failed.');
end
...
try
    dissimilarities = RSA.computeRDM.computeEuclideanRDM(X, Y, 'num_permutations', 5);
    disp('Test 2 passed.');
catch
    disp('Test 2 failed.');
end
...
try
    dissimilarities = RSA.computeRDM.computeEuclideanRDM(X, Y, 'rngType', 42);
    disp('Test 3 passed.');
catch
    disp('Test 3 failed.');
end
...
try
    dissimilarities = RSA.computeRDM.computeEuclideanRDM(X, Y, 'rngType', {'shuffle', 'twister'});
    disp('Test 4 passed.');
catch
    disp('Test 4 failed.');
end
...
try
    dissimilarities = RSA.computeRDM.computeEuclideanRDM(X(1:10, :), Y_mismatch);
...
try
    dissimilarities = RSA.computeRDM.computeEuclideanRDM(X_zero, Y);
    disp('Test 7 passed.');
catch
    disp('Test 7 failed.');
end
...
try
    dissimilarities = RSA.computeRDM.computeEuclideanRDM(X, Y_single_class);
...
X_empty = [];
try
    dissimilarities = RSA.computeRDM.computeEuclideanRDM(X_empty, Y);
...
try
    dissimilarities = RSA.computeRDM.computeEuclideanRDM(X_high_dim, Y);
    disp('Test 10 passed.');
catch
    disp('Test 10 failed.');
end
