% test_computeCMRDM.m
% This script tests the functionality of RDM_Computation.computeCMRDM()
...
try
    [RDM, params] = RDM_Computation.computeCMRDM(nonSquareMatrix);
...
emptyMatrix = [];
try
    [RDM, params] = RDM_Computation.computeCMRDM(emptyMatrix);
...
try
    [RDM, params] = RDM_Computation.computeCMRDM(zeroMatrix);
    disp('Test 18 passed.');
catch
    disp('Test 18 failed.');
end
...
try
    [RDM, params] = RDM_Computation.computeCMRDM(diagonalMatrix);
    disp('Test 19 passed.');
catch
    disp('Test 19 failed.');
end
...
try
    [RDM, params] = RDM_Computation.computeCMRDM(largeMatrix);
    disp('Test 20 passed.');
catch
    disp('Test 20 failed.');
end
